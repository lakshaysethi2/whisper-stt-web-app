...
def test_chunked_upload_flow():
    ...
    try:
        ...
        with patch("app.main.transcribe_audio",
            ...
            resp = client.post(f"/api/upload/finish/{upload_id}", params={"language": "en", "model": "base"}, timeout=2.0)
            assert resp.status_code == 200
            data = resp.json()
...
def test_missing_chunks_returns_detail_and_preserves_session():
    ...
    with patch("app.main.CHUNK_SIZE", 10), patch("app.main.MAX_CHUNKS", 100):
        ...
        try:
            ...
            resp = client.post(f"/api/upload/finish/{upload_id}", params={"model": "base"}, data={})
...
def test_size_mismatch_rejected():
    ...
    with patch("app.main.CHUNK_SIZE", 10), patch("app.main.MAX_CHUNKS", 100):
        ...
        try:
            ...
            with patch("app.main.transcribe_audio"):
                resp = client.post(f"/api/upload/finish/{upload_id}", params={"model": "base"}, data={"language": "en"})
                assert resp.status_code == 400
...
# ---------------------------------------------------------------------------
# Mandatory model choice tests
# ---------------------------------------------------------------------------


def test_transcribe_missing_model_rejected():
    """POST /api/transcribe without model returns 400."""
    resp = client.post(
        "/api/transcribe",
        files={"file": ("test.wav", b"X" * 1024, "audio/wav")},
        data={"language": "en"},
    ...
    assert resp.status_code == 400
    detail = resp.json()["detail"]
    assert "model" in detail.lower() or "choose" in detail.lower()


def test_transcribe_empty_model_rejected():
    """POST /api/transcribe with empty model string returns 400."""
    resp = client.post(
        "/api/transcribe",
        files={"file": ("test.wav", b"X" * 1024, "audio/wav")},
        data={"language": "en", "model": ""},
    )
    assert resp.status_code == 400
    detail = resp.json()["detail"]
    assert "model" in detail.lower() or "choose" in detail.lower()
...
def test_transcribe_invalid_model_rejected():
    """POST /api/transcribe with invalid model name returns 400."""
    resp = client.post(
        "/api/transcribe",
        files={"file": ("test.wav", b"X" * 1024, "audio/wav")},
        data={"language": "en", "model": "nonexistent-model"},
    )
    assert resp.status_code == 400
    detail = resp.json()["detail"]
    assert "Invalid model" in detail or "invalid" in detail.lower()


def test_transcribe_with_valid_model_accepted():
    """POST /api/transcribe with valid model is accepted."""
    from app.main import _jobs
    fast_result = {
        "text": "model test ok", "language": "en",
        "duration": 0.5, "process_time": 0.05,
        "segments": [], "id": "x",
        "device": "cpu", "compute_type": "int8",
    }
    with patch("app.main.transcribe_audio", side_effect=_make_async_mock(fast_result)):
        ...
        for _ in range(50):
            ...
            if r.status_code == 200 and r.json().get("status") == "completed":
                ...
                break
        assert status is not None, "job never completed"
        assert status["result"]["text"] == "model test ok"
        _jobs.pop(job_id, None)


def test_chunked_upload_missing_model_rejected():
    """POST /api/upload/finish without model returns 400."""
    with patch("app.main.CHUNK_SIZE", 10), patch("app.main.MAX_CHUNKS", 100):
        upload_id = None
        try:
            resp = client.post(
...
def test_api_models_returns_ui_choices():
    """GET /api/models includes ui_choices field."""
    resp = client.get("/api/models")
    assert resp.status_code == 200
    data = resp.json()
    assert "ui_choices" in data
    assert "base" in data["ui_choices"]
    assert "large-v3-turbo" in data["ui_choices"]


def test_spa_job_route_still_works():
    """GET /j/{job_id} still serves index.html with model choice present."""
    resp = client.get("/j/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
    assert resp.status_code == 200
    assert resp.headers["content-type"].startswith("text/html")
    assert b"Choose a model" in resp.content


def test_browser_stt_links_present():
    """Index page contains browser STT links."""
    resp = client.get("/")
    assert resp.status_code == 200
    html = resp.content.decode()
    assert "Xenova/whisper-web" in html
    assert "Xenova/whisper-webgpu" in html
    assert "huggingface.co/spaces/Xenova/whisper-web" in html
    assert "huggingface.co/spaces/Xenova/whisper-webgpu" in html


def test_browser_stt_note_present():
    """Index page contains the note about browser STT running on device."""
    resp = client.get("/")
    assert resp.status_code == 200
    html = resp.content.decode()
    assert "your device" in html
    assert "not this server" in html
