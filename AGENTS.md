...
## Node picker + GPU worker visibility

- **Worker dispatch**: `WHISPER_WORKER_URLS` (comma-separated) + `WHISPER_WORKER_NAMES` (display names, order-matched) in `app/remote.py`. Auto dispatch = round-robin over workers for models in `WHISPER_GPU_MODELS` (tiny/base/small/crisperwhisper-small — the laptops' ~2GB Maxwell/Pascal VRAM runs float32 only), silent local-CPU fallback on any failure. Explicit `worker` param (name or 1-based index) on `/api/transcribe` (Form) + `/api/upload/finish` (Query) routes EXCLUSIVELY to that node — failure FAILS the job with "Node X failed: … Retry or choose another node." (no fallback). `should_dispatch(model, worker)` in remote.py: explicit node overrides the model whitelist.
- **Live visibility**: `GET /api/workers` → `[{name, url, online, device}]` (parallel health checks, `_worker_devices` caches device for "(GPU)" labels). Job dicts carry live `node` + `stage` (uploaded → dispatched → transcribing → done/failed), set via `_set_job_live` under `_jobs_lock`; status endpoint returns them (defaults: "server (CPU)"/"waiting" for old persisted jobs). Frontend: `static/app.js` renders pill-style node picker (`#node-picker`) from /api/workers + 30s re-poll + offline banner; `statusMessage()` renders "Processing on {node} — …"; `#result-node` badge on results.
- **Worker image**: `Dockerfile.worker` (NOT the stock Dockerfile) — the ctranslate2 pip wheel ships only sm_86/90/100 kernels, so the laptops' GPUs need a from-source build: multi-stage, builder compiles CTranslate2 v4.8.1 (`git clone --branch v4.8.1`, `cmake … -DWITH_MKL=OFF -DOPENMP_RUNTIME=COMP -DWITH_CUDA=ON -DCUDA_DYNAMIC_LOADING=ON -DCUDA_ARCH_LIST=<5.0|6.1>`, then `pip install ./ct2/python`), runtime copies `/usr/local/lib64/libctranslate2.so*` + site-packages `ctranslate2` (python 3.10 on ubuntu22.04 both stages). `docker-compose.worker.yml` uses it with `WORKER_CUDA_ARCH` build arg. torch/crisperwhisper excluded — crisperwhisper-* jobs stay on the VPS in auto mode.
- **VPS networking for workers**: laptop reverse tunnels bind the whisper-net gateway `10.99.0.1` (`GatewayPorts clientspecified` sshd drop-in + `ufw allow in on br-055ed2d393a3 to any port 8564,8565`); compose `extra_hosts: host.docker.internal:10.99.0.1` (NOT host-gateway — resolves to iptables-isolated 172.17.0.1).
